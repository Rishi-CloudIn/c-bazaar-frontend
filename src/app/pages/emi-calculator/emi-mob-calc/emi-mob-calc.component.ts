...
@Component({
  selector: 'app-emi-mob-calc',
  templateUrl: './emi-mob-calc.component.html',
  styleUrls: ['./emi-mob-calc.component.css'],
})
export class EmiMobCalcComponent implements OnInit {
  ...
  intrestCalculation(duration: number, prin: number, inter: number): any {
    ...
    for (
      let i = 1;
      duration % 12 == 0 ? i - 1 <= duration / 12 : i - 1 <= duration / 12 + 1;
      i++
    ) {
      let tempObj: any = {
        year: year,
      ...
      for (let j = 1; j <= 12; j++) {
        ...
        if (tempObj2.balance >= 0) tempObj.monthData.push(tempObj2);
      ...
      tempObj.balance = tempObj.monthData[tempObj.monthData.length - 1].balance;
